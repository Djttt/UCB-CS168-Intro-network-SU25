...
class StudentUSocket(StudentUSocketBase):
  ...
  def update_rto(self, acked_pkt):
    ...
    # clamp function. Clamp self.rto so that it is 
    # at most self.MAX_RTO, and at least self.MIN_RTO
  ...
  def handle_accepted_fin(self, seg):
    ...
    if self.state in [ESTABLISHED]:
      rcv.nxt = rcv.nxt |PLUS| len(seg.payload)
      self.set_pending_ack()
    ...
    if self.state == FIN_WAIT_1:
      ...
      if seg.FIN:
        self.rcv.nxt = self.rcv.nxt |PLUS| len(seg.payload)
        self.set_pending_ack()
        self._delete_tcb()
  ...
  def check_timer_retx(self):
    ...
    if time_in_queue > self.rto:
      ...
      ## Start of Stage 9.3 ##
      self.rto *= 2
      self.rto = max(self.MAX_RTO, self.rto)
      ## End of Stage 9.3 ##
